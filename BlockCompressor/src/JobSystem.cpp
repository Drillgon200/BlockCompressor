...
namespace job {
	...
	void JobSystem::init_job_system(uint32_t threadCount) {
		...
		for (uint32_t i = 0; i < threadCount; i++) {
			...
			//SetThreadAffinityMask(threadpool[i].native_handle(), static_cast<DWORD_PTR>(1) << (i * 2));
			//SYSTEM_LOGICAL_PROCESSOR_INFORMATION buf;
			//DWORD len;
			//BOOL res = GetLogicalProcessorInformation(&buf, &len);
#elif __linux__
			cpu_set_t cputset;
			CPU_ZERO(&cpuset);
			CPU_SET(i, &cpuset);
	...
	void JobSystem::threadFunc(uint32_t idx) {
		...
		delete threadData;
		threadData = nullptr;
		std::cout << "Exited job thread " << idx << " successfully\n";
	}
